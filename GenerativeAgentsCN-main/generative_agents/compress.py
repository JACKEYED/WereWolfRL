# 文件作用：将狼人杀检查点压缩成网页回放所需的 movement.json 和文字版 simulation.md。
...
def get_checkpoint_files(checkpoints_folder):
    files = sorted(os.listdir(checkpoints_folder))
    json_files = []
    for file_name in files:
        if file_name.endswith(".json") and file_name != "conversation.json" and file_name != "werewolf_state.json":
            json_files.append(os.path.join(checkpoints_folder, file_name))
    return json_files


def get_agent_names(checkpoints_folder):
    json_files = get_checkpoint_files(checkpoints_folder)
    if not json_files:
        return []
    with open(json_files[0], "r", encoding="utf-8") as f:
        json_data = json.load(f)
    return list(json_data.get("agents", {}).keys())
...
def generate_report(checkpoints_folder, compressed_folder, compressed_file):
    last_state = dict()
    last_werewolf_record_count = 0
    agent_names = get_agent_names(checkpoints_folder)
    ...
    def extract_action(json_data):
        nonlocal last_werewolf_record_count
        markdown_content = ""
        agents = json_data["agents"]
        for agent_name, agent_data in agents.items():
        ...
        if json_data['time'] not in conversation.keys():
            return markdown_content + extract_werewolf_records(json_data)
    ...
    def extract_werewolf_records(json_data):
        nonlocal last_werewolf_record_count
        werewolf = json_data.get("werewolf")
        if not werewolf:
            return ""
        records = werewolf.get("phase_records", [])
        new_records = records[last_werewolf_record_count:]
        last_werewolf_record_count = len(records)
        if not new_records:
            return ""

        markdown_content = "## 狼人杀事件记录：\n\n"
        for record in new_records:
            scope = record.get("scope", "")
            phase = record.get("phase", "")
            text = record.get("text", "")
            markdown_content += f"- `{scope}` {phase}：{text}\n"
        markdown_content += "\n"
        return markdown_content

    all_markdown_content = extract_description()
    for file_path in get_checkpoint_files(checkpoints_folder):
