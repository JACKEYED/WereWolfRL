...
def build_agent_prompt(director, name: str, phase: str, task: str, *, response_kind: str) -> str:
    """构造给 Agent 的完整 prompt：身份 + 公开/私密上下文 + 当前 belief + 任务 + 输出格式约束。"""
    ...
    # 场上压力
    alive_count = len(director.alive_names())
    wolves_alive = len(director.alive_names(role="werewolf"))
    good_alive = alive_count - wolves_alive
    pressure_block = (
        f"\n场上：第 {director.day} 日，存活 {alive_count} 人"
        f"（你的真实身份只有你自己看到）。"
    )
...
3. 把非正式对话当成真实记忆使用。
4. 在自由发言里，每句话应当**显式或隐式**地服务于一个具体意图——指认、暗示、试探、洗白、转移火力之一，不要只描述天气或市井氛围。
5. {output_spec}
"""
